...
for i in range(431):
    if(type(ahf_prog['Event 1'].astype('str')[i]) != pd._libs.missing.NaT):
        eve.iloc[0][i] = 1
    if(type(ahf_prog['Event 2'].astype('str')[i]) != pd._libs.missing.NaT):
        eve.iloc[1][i] = 1
    if(type(ahf_prog['Event 3'].astype('str')[i]) != pd._libs.missing.NaT):
        eve.iloc[2][i] = 1
    if(type(ahf_prog['Event 4'].astype('str')[i]) != pd._libs.missing.NaT):
        eve.iloc[3][i] = 1
...
X_100=np.empty([1520,0])
for i in range(100):
    X_100=np.append(X_100, np.array(AHF_data.iloc[:,int(i*10)]).reshape(1520,1), axis=1)
#------------2021/12/14




AHF_data = pd.concat([AHF_data[np.arange(1000)], AHF_data[1002]],axis=1)
AHF_data= AHF_data.rename(columns={1002:1000})
array = np.empty([0,1006])
